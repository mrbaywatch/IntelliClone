...
begin
    return exists (select 1
                   ...
                   where public.chatbots.id = chatbot_id
                     and public.has_role_on_account(public.chatbots.account_id));
end;
$$
    language plpgsql;
...
$$
begin
    return query select subscription.period_starts_at,
                 ...
                 group by subscription.period_starts_at, subscription.period_ends_at, item.interval_count,
                          item.variant_id, plan.max_documents, plan.max_messages, plan.max_chatbots;
end;
$$ language plpgsql;
