...
function ModalForm(
  ...
  const { currentStep, filters } = useWatch({
  ...
  const getFilters = () => {
    ...
    const disallow = (filters?.disallow ?? [])
      .map((filter) => filter.value)
      .filter((value) => !!value) as string[];
  ...
  return (
    <div className={'flex flex-col space-y-12'}>
      <Stepper steps={steps} currentStep={currentStep || 0} />
...
function ConfirmWebsiteStep(
  ...
  return (
    <div className={'animate-in fade-in flex flex-col space-y-4'}>
      ...
      <div>
        ...
        <pre
          className={'bg-muted text-muted-foreground mt-2 border p-4 text-xs'}
        >
          <code>{props.url}</code>
        </pre>
      </div>
...
function AnalyzeWebsiteSitemapStep(
  ...
  if (props.isCreatingJob) {
    return (
      <div
        className={
          'animate-in fade-in flex flex-col items-center justify-center space-y-4 text-sm'
        }
      >
        <Spinner />
  ...
  return (
    <div className={'animate-in fade-in flex flex-col space-y-4 text-sm'}>
      <div className={'flex flex-col space-y-2'}>
        <p
...
function useStartCrawlingJob() {
  ...
  const mutationFn = async (params: Params) => {
    try {
      await createChatbotCrawlingJobAction(params);
