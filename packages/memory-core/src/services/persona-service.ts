 ...
 */
export class InMemoryPersonaStorage implements PersonaStorage {
  private personas: Map<string, UserPersona> = new Map();
  
  private getKey(userId: string, tenantId: string, chatbotId?: string): string {
    return `${tenantId}:${userId}:${chatbotId ?? 'global'}`;
  }
  
  async get(userId: string, tenantId: string, chatbotId?: string): Promise<UserPersona | null> {
    const key = this.getKey(userId, tenantId, chatbotId);
    return this.personas.get(key) ?? null;
  }
  
  async save(persona: UserPersona): Promise<UserPersona> {
  ...
  async update(personaId: string, updates: Partial<UserPersona>): Promise<UserPersona> {
  ...
  async listByTenant(tenantId: string, limit = 100): Promise<UserPersona[]> {
    const results: UserPersona[] = [];
...
const DEFAULT_CONFIG: PersonaServiceConfig = {
...
export class PersonaService {
  private readonly config: PersonaServiceConfig;
  
  constructor(
    private readonly storage: PersonaStorage,
    config: Partial<PersonaServiceConfig> = {}
  ) {
   ...
   */
  private applyInsightToPersona(persona: UserPersona, insight: ConversationInsight): void {
    switch (insight.type) {
      case 'fact':
   ...
   */
  private addFact(persona: UserPersona, fact: string, confidence: number): void {
    // Categorize the fact
   ...
   */
  private updatePreference(persona: UserPersona, insight: ConversationInsight): void {
   ...
   */
  private updateCommunicationStyle(persona: UserPersona, insight: ConversationInsight): void {
    ...
    if (insight.personaField) {
      ...
      if (field[0] === 'communicationStyle' && field[1]) {
        const key = field[1] as keyof CommunicationStyle;
        if (typeof persona.communicationStyle[key] === 'number') {
          // Adjust based on insight confidence
          const currentValue = persona.communicationStyle[key] as number;
          const adjustment = insight.confidence > 0.8 ? 0.15 : 0.05;
          (persona.communicationStyle[key] as number) = 
   ...
   */
  private addRelationship(persona: UserPersona, content: string): void {
   ...
   */
  private addGoal(persona: UserPersona, content: string): void {
   ...
   */
  private addChallenge(persona: UserPersona, content: string): void {
   ...
   */
  private calculateOverallConfidence(persona: UserPersona): number {
    let factors = 0;
   ...
   */
  private getOpeningLine(purpose: string, formal: boolean, language: string): string {
   ...
   */
  private getClosingLine(purpose: string, formal: boolean, language: string): string {
    if (language === 'no') {
      if (formal) {
   ...
   * Select signoff
   */
  private selectSignoff(style: CommunicationStyle, formal: boolean): string {
   ...
   */
  private generateSubject(purpose: string, formal: boolean): string {
   ...
   * Get a persona by ID
   */
  private async getPersonaById(personaId: string): Promise<UserPersona | null> {
  ...
  analyzeMessageStyle(message: string): Partial<CommunicationStyle> {
    ...
    const norwegianCount = norwegianWords.filter(w => 
      new RegExp(`\\b${w}\\b`, 'i').test(message)
    ).length;
   ...
   */
  getQuestionText(question: ProbingQuestion, language: 'no' | 'en' = 'no'): string {
