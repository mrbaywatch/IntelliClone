 ...
 */
export const DEFAULT_MEMORY_SERVICE_CONFIG = {
...
export class MemoryService {
    ...
    importanceScorer;
    config;
    constructor(storage, embedding, importanceScorer, config = {}) {
    ...
    async store(input) {
        ...
        const importanceScore = await this.importanceScorer.scoreContent(input.content, input.type, input.source);
        // Check minimum importance threshold
        if (importanceScore.score < this.config.minImportanceToStore) {
            throw new MemoryRejectedError(`Memory importance score ${importanceScore.score} below threshold ${this.config.minImportanceToStore}`);
        }
        // Check for duplicates
        const duplicates = await this.findDuplicates(input.userId, input.tenantId, embeddingResult.embedding, this.config.deduplicationThreshold);
    ...
    async retrieve(query, options = {}) {
        ...
        const candidates = await this.storage.vectorSearch(queryEmbedding.embedding, query.userId, query.tenantId, {
    ...
    async consolidate(options) {
        ...
        const memories = await this.storage.getForConsolidation(options.tenantId, options.userId, {
        ...
        if (options.mergeSimilar && !options.dryRun) {
            const mergeResults = await this.mergeSimilarMemories(memories, options.mergeThreshold ?? 0.95);
    ...
    async forget(criteria) {
        ...
        for (const memory of candidates) {
            ...
            if (criteria.skipHighImportance &&
            ...
            if (criteria.containsKeywords &&
                !criteria.containsKeywords.some(kw => memory.content.toLowerCase().includes(kw.toLowerCase()))) {
                continue;
            }
            // Execute deletion
    ...
    async update(memoryId, update) {
        ...
        if (update.content && update.content !== memory.content) {
            ...
            const importance = await this.importanceScorer.scoreContent(update.content, memory.type, memory.metadata.source);
            updates.importanceScore = importance.score;
        }
        return this.storage.update(memoryId, updates);
    }
    // ==================== Private Helpers ====================
    async findDuplicates(userId, tenantId, embedding, threshold) {
        const results = await this.storage.vectorSearch(embedding, userId, tenantId, {
            limit: 5,
    ...
    async reinforceMemory(memoryId, input, newScore) {
        const memory = await this.storage.get(memoryId);
        if (!memory) {
            throw new MemoryNotFoundError(memoryId);
        }
    ...
    applyDiversitySampling(memories, threshold) {
        const selected = [];
        for (const candidate of memories) {
            ...
            const tooSimilar = selected.some(selected => {
                ...
                const similarity = this.cosineSimilarity(candidate.memory.embedding.vector, selected.memory.embedding.vector);
    ...
    determineConsolidationAction(memory, decayScore) {
    ...
    recordAction(result, action, memory) {
        switch (action.type) {
            case 'promote':
                result.promoted.push({
                    memoryId: memory.id,
    ...
    async mergeSimilarMemories(memories, threshold) {
        ...
        for (const memory of memories) {
            ...
            const similar = memories.filter(other => {
                ...
                const similarity = this.cosineSimilarity(memory.embedding.vector, other.embedding.vector);
                return similarity >= threshold;
            });
            if (similar.length > 0) {
    ...
    mergeContent(memories) {
        ...
        return memories.reduce((longest, current) => current.content.length > longest.length ? current.content : longest, '');
    }
    async triggerConsolidation(tenantId, userId) {
