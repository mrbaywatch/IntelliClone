import { createSystemPrompt, SystemPromptBuilder } from './system-prompt.js';
...
export const PromptTemplates = {
   ...
   */
  assistant: (options?: { name?: string; locale?: string }): SystemPromptBuilder => {
...
You provide clear, concise, and helpful responses.`);
   ...
   */
  codeAssistant: (options?: { languages?: string[]; framework?: string }): SystemPromptBuilder => {
...
- Provide insights and recommendations based on data`)
   ...
   */
  customerSupport: (options?: { companyName?: string; productName?: string }): SystemPromptBuilder => {
...
- Escalate complex issues when necessary`)
   ...
   * Translator prompt
   */
  translator: (options: { sourceLanguage?: string; targetLanguage: string }): SystemPromptBuilder => {
...
- Maintain proper grammar and punctuation`);
   ...
   * Summarizer prompt
   */
  summarizer: (options?: { style?: 'brief' | 'detailed' | 'bullet-points' }): SystemPromptBuilder => {
...
- Maintain accuracy and avoid adding information not present in the original`)
   ...
   */
  norwegianAssistant: (options?: { bokmaal?: boolean }): SystemPromptBuilder => {
...
export function createProductPrompt(
  product: 'intelli-gym' | 'intelli-track' | 'intelli-diet',
  userContext?: { name?: string; locale?: string }
): SystemPromptBuilder {
