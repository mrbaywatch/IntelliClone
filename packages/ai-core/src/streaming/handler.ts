...
): Promise<{ content: string; usage: TokenUsage | null; error: Error | null }> {
 ...
 */
export async function* createBufferedStream(
  generator: AsyncGenerator<StreamChunk>,
  bufferBy: 'word' | 'sentence' = 'word'
): AsyncGenerator<StreamChunk> {
  let buffer = '';
 ...
 */
export function createTimedCallbacks(
  callbacks: StreamCallbacks
): StreamCallbacks & { getMetrics: () => StreamMetrics } {
