...
export class ModelRouter {
     ...
     */
    selectModel(task, preferences) {
        const availableModels = this.getAvailableModels();
        if (availableModels.length === 0) {
            throw new ProviderError('No providers are configured. Please set OPENAI_API_KEY or ANTHROPIC_API_KEY.', 'router', 'NO_PROVIDERS');
        }
        // Filter by task support
        let candidates = availableModels.filter((m) => m.supportedTasks.includes(task));
        ...
        if (preferences?.preferredProvider) {
            const filtered = candidates.filter((m) => m.provider === preferences.preferredProvider);
        ...
        if (preferences?.maxCost !== undefined) {
            candidates = candidates.filter((m) => m.outputCostPer1k <= preferences.maxCost);
        }
        if (candidates.length === 0) {
            throw new ProviderError('No models match the specified requirements', 'router', 'NO_MATCHING_MODELS');
        }
     ...
     */
    scoreModel(model, task, preferredTier) {
        let score = 0;
        // Tier scoring
        if (preferredTier) {
     ...
     */
    async chat(request, task = 'chat', preferences) {
        const model = this.selectModel(task, preferences);
        const provider = this.providers.get(model.provider);
        if (!provider) {
            throw new ProviderError(`Provider ${model.provider} not found`, 'router', 'PROVIDER_NOT_FOUND');
        }
     ...
     */
    async streamChat(request, callbacks, task = 'chat', preferences) {
     ...
     */
    async *streamChatGenerator(request, task = 'chat', preferences) {
        const model = this.selectModel(task, {
            ...preferences,
            requireStreaming: true,
        });
     ...
     */
    async chatWithModel(request, modelId) {
        const allModels = this.getAllModels();
        const model = allModels.find((m) => m.modelId === modelId);
        if (!model) {
            throw new ProviderError(`Model ${modelId} not found`, 'router', 'MODEL_NOT_FOUND');
        }
        const provider = this.providers.get(model.provider);
        if (!provider) {
            throw new ProviderError(`Provider ${model.provider} not found`, 'router', 'PROVIDER_NOT_FOUND');
        ...
        if (!provider.isConfigured()) {
            throw new ProviderError(`Provider ${model.provider} is not configured`, 'router', 'PROVIDER_NOT_CONFIGURED');
        }
