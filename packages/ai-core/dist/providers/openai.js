...
export class OpenAIProvider extends BaseLLMProvider {
    ...
    getClient() {
        if (!this.client) {
            ...
            if (!apiKey) {
                throw new ProviderError('OPENAI_API_KEY is not set', 'openai', 'MISSING_API_KEY');
            }
    ...
    async chat(request, modelId) {
        ...
        if (!model) {
            throw new ProviderError(`Model ${modelId} not found`, 'openai', 'MODEL_NOT_FOUND');
        }
        try {
    ...
    async streamChat(request, callbacks, modelId) {
        ...
        if (!model.supportsStreaming) {
            throw new ProviderError(`Model ${model.modelId} does not support streaming`, 'openai', 'STREAMING_NOT_SUPPORTED');
        }
        try {
            const client = this.getClient();
            const messages = request.systemPrompt
