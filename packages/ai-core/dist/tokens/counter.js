...
export class TokenCounter {
     ...
     */
    static estimate(text, language = 'en') {
        if (!text)
            return 0;
     ...
     */
    static estimateMessages(messages, language = 'en') {
        let total = 0;
     ...
     */
    static estimateCost(usage, model) {
     ...
     */
    static estimateRequestCost(messages, systemPrompt, expectedOutputTokens, model, language = 'en') {
     ...
     */
    static checkContextLimit(messages, systemPrompt, model, language = 'en') {
 ...
 */
export function calculateBlendedRate(model, inputRatio = 0.7) {
 ...
 */
export function compareCosts(usage, models) {
    return models
        .map((model) => ({
        model,
